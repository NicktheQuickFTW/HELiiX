      ...
      <Card padding="m" background="surface" border="neutral-medium">
        <Column gap="s">
          ...
          <Text variant="body-default-s" onBackground="neutral-medium">
            These manuals contain conference-wide policies that apply to all sports:
          </Text>
          
          <Column gap="xs" style={{ marginLeft: '16px' }}>
            <Text variant="body-strong-s" onBackground="success-medium">✓ Will Extract:</Text>
            <Text variant="body-default-s">• Governance Structure & Policies</Text>
            <Text variant="body-default-s">• Television Broadcasting Standards</Text>
            <Text variant="body-default-s">• Academic Integrity Requirements</Text>
            <Text variant="body-default-s">• Faculty Athletic Representative Duties</Text>
            <Text variant="body-default-s">• General Championship Procedures</Text>
            <Text variant="body-default-s">• Member Institution Requirements</Text>
          </Column>
          
          <Text variant="body-default-s" onBackground="neutral-weak" style={{ fontStyle: 'italic' }}>
            These policies will be stored as conference-wide policies (not sport-specific).
          </Text>
        </Column>
      </Card>
                  ...
                  <Column gap="xs">
                    ...
                    {result.section && (
                      <Text variant="body-default-xs" onBackground="neutral-weak">
                        {result.section} {result.category && `• ${result.category}`}
                      </Text>
                    )}
                    {result.error && (
      ...
      {manuals.some(m => !m.exists) && (
        <Card padding="m" background="warning-weak" border="warning-medium">
          <Column gap="s">
            ...
            <Text variant="body-default-s">
              Some conference manual files could not be found. Please verify the file paths:
            </Text>
            <Column gap="xs" style={{ marginLeft: '16px' }}>
              {manuals.filter(m => !m.exists).map(manual => (
                <Text key={manual.filename} variant="body-default-s" onBackground="warning-medium">
                  • {manual.filename}
                </Text>
              ))}
            </Column>
