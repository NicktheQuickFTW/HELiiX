...
export default function PolicyComparePage() {
  ...
  const comparePoliciesAcrossSports = async () => {
    ...
    try {
      ...
      const sportPolicies = selectedSports.map(sportId => {
        ...
        const sportPolicyList = policies.filter(p => 
      ...
      const policyTitles = [...new Set(
        sportPolicies.flatMap(sp => 
          sp.policies.map(p => p.title.replace(sp.sport?.sport_name || '', '').trim())
        )
      )]

      for (const baseTitle of policyTitles) {
        const matchingPolicies = sportPolicies.map(sp => ({
          sport: sp.sport,
          policy: sp.policies.find(p => 
            p.title.toLowerCase().includes(baseTitle.toLowerCase()) ||
            baseTitle.toLowerCase().includes(p.title.replace(sp.sport?.sport_name || '', '').trim().toLowerCase())
          )
        })).filter(mp => mp.policy)
  ...
  return (
    <Column fillWidth gap="l" padding="xl">
      ...
      <Row fillWidth justifyContent="space-between" alignItems="center">
        <Column gap="xs">
          ...
          <Text variant="body-default-s" onBackground="neutral-weak">
            Compare policies across sports to identify differences and standardization opportunities
          </Text>
        </Column>
      </Row>
        ...
        <Column gap="m">
          ...
          <Grid columns="2" gap="m">
            ...
            <Column gap="xs">
              ...
              <Column gap="xs" style={{ maxHeight: '200px', overflowY: 'auto' }}>
                {sports.map(sport => (
                  <Row key={sport.sport_id} alignItems="center" gap="s">
                    <input
                      type="checkbox"
                      checked={selectedSports.includes(sport.sport_id.toString())}
                      onChange={() => toggleSportSelection(sport.sport_id.toString())}
                    />
          ...
          <Row gap="s">
            <Button 
              ...
              disabled={comparing || !selectedCategory || selectedSports.length < 2}
            >
          ...
          {comparisonResults.map((result, index) => (
            <Card key={index} padding="m" background="surface" border="neutral-medium">
              <Column gap="m">
                <Row justifyContent="space-between" alignItems="center">
                  <Column gap="xs">
                    ...
                    <Text variant="body-default-xs" onBackground="neutral-weak">
                      {getCategoryDisplay(result.category)} • {result.policies.length} sports
                    </Text>
                  </Column>
                  
                  <Badge 
                    variant={result.differences.length > 0 ? "warning" : "success"}
                    size="s"
                  >
                ...
                {result.differences.length > 0 && (
                  <Column gap="s">
                    <Text variant="label-strong-xs">Identified Differences:</Text>
                    {result.differences.map((diff: any, diffIdx: number) => (
                      <Card key={diffIdx} padding="s" background="warning-weak" border="warning-medium">
                        <Column gap="xs">
                          <Text variant="body-strong-s">{diff.description}</Text>
                          <Text variant="body-default-xs">Type: {diff.type}</Text>
                          {diff.details && Array.isArray(diff.details) && (
                            <Column gap="xs" style={{ marginTop: '8px' }}>
                              {diff.details.map((detail: any, detailIdx: number) => (
                                <Text key={detailIdx} variant="body-default-xs" onBackground="neutral-weak">
                                  {detail.sport}: {detail.summary || detail.length || 'No content'}
                                </Text>
                              ))}
                            </Column>
                          )}
          ...
          <Card padding="m" background="surface" border="neutral-medium">
            <Column gap="s">
              ...
              <Grid columns="3" gap="m">
                ...
                <Column gap="xs">
                  <Text variant="heading-strong-l">
                    {comparisonResults.filter(r => r.differences.length > 0).length}
                  </Text>
                  <Text variant="body-default-s" onBackground="neutral-weak">
                    With Differences
                  </Text>
                ...
                <Column gap="xs">
                  <Text variant="heading-strong-l">
                    {Math.round((comparisonResults.filter(r => r.differences.length === 0).length / comparisonResults.length) * 100)}%
                  </Text>
                  <Text variant="body-default-s" onBackground="neutral-weak">
                    Consistency Rate
                  </Text>
      ...
      {/* No Results */}
      {comparisonResults.length === 0 && selectedCategory && selectedSports.length >= 2 && !comparing && (
        <Card padding="xl" background="surface" border="neutral-medium">
          <Column alignItems="center" gap="m">
            <Text variant="heading-strong-m">No comparable policies found</Text>
            <Text variant="body-default-s" onBackground="neutral-weak">
              The selected sports may not have policies in the {getCategoryDisplay(selectedCategory)} category, 
              or the policies may need to be parsed first.
            </Text>
          </Column>
        </Card>
      )}
