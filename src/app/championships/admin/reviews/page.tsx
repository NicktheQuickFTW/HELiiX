...
export default function CredentialReviewsPage() {
  ...
  const fetchRequests = async () => {
    try {
      ...
      const underReview =
        data?.filter((r) => r.status === 'under_review').length || 0;
  ...
  const applyFilters = () => {
    ...
    if (typeFilter !== 'all') {
      filtered = filtered.filter(
        (request) => request.credential_type === typeFilter
      );
    }
    ...
    if (eventFilter !== 'all') {
      filtered = filtered.filter(
        (request) => request.championship_event_id === eventFilter
      );
    }
  ...
  const handleReviewRequest = async () => {
    ...
    try {
      const updateData = {
        ...
        ...(reviewAction.action === 'deny' && {
          denial_reason: reviewAction.notes,
        }),
      };
      ...
      if (reviewAction.action === 'approve') {
        const credentialData = {
          ...
          access_levels:
            reviewAction.accessLevels ||
            selectedRequest.requested_access_levels,
          valid_from:
            selectedRequest.championship_events.start_date + 'T00:00:00Z',
          valid_until:
            selectedRequest.championship_events.start_date + 'T23:59:59Z',
          holder_name: `${selectedRequest.first_name} ${selectedRequest.last_name}`,
          holder_title: selectedRequest.title,
          organization_name:
            selectedRequest.organizations?.name || 'Independent',
          issued_by: user.id,
          status: 'active',
        };
  ...
  const generateCredentialNumber = async () => {
    ...
    return (
      data ||
      `B12-${new Date().getFullYear()}-${Math.random().toString().substr(2, 6)}`
    );
  };
              ...
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <Text variant="body-sm" muted>
                    {request.credential_type.replace('_', ' ')} •{' '}
                    {request.championship_events.name}
                  </Text>
                </div>
            ...
            <Column>
              <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4 mb-4">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div className="text-sm">
                    <p className="font-medium">
                      {request.organizations?.name || 'Independent'}
                    </p>
                    <p className="text-muted-foreground">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div className="text-sm">
                    <p className="font-medium">
                      {request.requested_access_levels.length} access levels
                    </p>
                  ...
                  <Dialog className="max-w-4xl max-h-[80vh] overflow-y-auto">
                    <Column gap="xs">
                      ...
                      <Text variant="body-sm" muted>
                        ...
                        {selectedRequest?.championship_events.name}
                      </Text>
                    </Column>
                      ...
                      <div className="space-y-6">
                          ...
                          <div>
                            ...
                            <div className="space-y-2 text-sm">
                              ...
                              <div>
                                ...
                                {selectedRequest.title || 'Not provided'}
                              </div>
                            </div>
                          </div>
                        ...
                        <div>
                          ...
                          <div className="space-y-2 text-sm">
                            <div>
                              <span className="font-medium">Type:</span>{' '}
                              {selectedRequest.credential_type.replace(
                                '_',
                                ' '
                              )}
                            </div>
                            ...
                            {reviewAction.action === 'approve' && (
                              <div className="space-y-2">
                                ...
                                <p className="text-sm text-muted-foreground mb-2">
                                  Select which access levels to grant (defaults
                                  to requested levels)
                                </p>
