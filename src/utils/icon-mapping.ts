 ...
 */
export const getSemanticReplacement = (
  name: string,
  context?: string
): IconName => {
  ...
  if (context) {
    if (context.includes('nav') || context.includes('link'))
      return 'chevronRight';
    if (context.includes('action') || context.includes('button')) return 'plus';
    if (context.includes('alert') || context.includes('warning'))
      return 'warning';
