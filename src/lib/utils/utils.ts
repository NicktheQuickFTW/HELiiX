...
export function getAdjacentPages(
  ...
  try {
    ...
    if (isTopLevelPage) {
      ...
      const currentIndex = sortedTopLevelPages.findIndex(
      ...
      const prevPage =
        currentIndex > 0 ? sortedTopLevelPages[currentIndex - 1] : null;
      const nextPage =
        currentIndex < sortedTopLevelPages.length - 1
          ? sortedTopLevelPages[currentIndex + 1]
          : null;
    ...
    const globalCurrentIndex = allSortedPages.findIndex(
      (page) => page.slug === currentSlug
    );
    ...
    const globalPrevPage =
      globalCurrentIndex > 0 ? allSortedPages[globalCurrentIndex - 1] : null;
    const globalNextPage =
      globalCurrentIndex < allSortedPages.length - 1
        ? allSortedPages[globalCurrentIndex + 1]
        : null;
    ...
    const sectionCurrentIndex = sortedSectionPages.findIndex(
      (page) => page.slug === currentSlug
    );
    ...
    const isLastInSection =
      sectionCurrentIndex === sortedSectionPages.length - 1;

    // Get section-specific previous and next pages
    const sectionPrevPage =
      sectionCurrentIndex > 0
        ? sortedSectionPages[sectionCurrentIndex - 1]
        : null;
    const sectionNextPage =
      sectionCurrentIndex < sortedSectionPages.length - 1
        ? sortedSectionPages[sectionCurrentIndex + 1]
        : null;
