...
export function AuthProvider({ children }: { children: React.ReactNode }) {
    ...
    try {
      ...
      if (error) {
        ...
        if (error.code === 'PGRST116' || error.code === '42P01') {
          console.log('User profiles table not found or user profile missing, using default profile')
  ...
  const createDefaultProfile = (userId: string): UserProfile => {
    return {
      id: userId,
      email: user?.email || 'nick.williams@big12sports.com',
      first_name: 'Nick',
      last_name: 'Williams',
      role: 'admin',
  ...
  const updateLastLogin = async (userId: string) => {
    try {
      ...
      if (error && error.code !== '42P01') {
        console.log('Could not update last login - table may not exist:', error.message)
      }
    } catch (error) {
  ...
  useEffect(() => {
    ...
    const initAuth = async () => {
      try {
        ...
        if (error) {
          console.log('No Supabase session found, using mock data for development')
  ...
  const updateProfile = async (updates: Partial<UserProfile>) => {
    try {
      ...
      if (!error) {
        ...
        await logUserAction('user:profile_update', 'user_profiles', user.id, { updates })
      }
  ...
  const logUserAction = async (
    ...
    try {
      ...
      if (error) {
        console.log('Could not log user action - function may not exist:', error.message)
      }
    } catch (error) {
