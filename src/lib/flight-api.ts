...
export class MockFlightAPI {
  ...
  private static getMockDepartureTime(
    date: string,
  ...
  private static getMockArrivalTime(
    date: string,
    timeOfDay: 'morning' | 'afternoon' | 'evening',
    durationMinutes: number
  ): string {
    const departure = new Date(
      `${date}T${this.getMockDepartureTime(date, timeOfDay).split('T')[1]}`
    );
...
export class Big12FlightAPI {
  static async searchFlights(
    ...
    try {
      ...
      if (
        amadeusFlights.status === 'fulfilled' &&
        amadeusFlights.value.length > 0
      ) {
