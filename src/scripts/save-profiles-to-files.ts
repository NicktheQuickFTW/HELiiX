...
async function savePlayerProfiles(
  ...
  const successfulPlayers = players.filter(
    (p) => p.success && p.research?.profile
  );
  ...
  const playersBySchool = playerProfiles.reduce(
    (acc, player) => {
  ...
  const allPlayersData = {
    metadata: {
      ...
      avgConfidence:
        playerProfiles.reduce((sum, p) => sum + p.confidence, 0) /
  ...
  result.filesCreated.push(
    'public/data/media-day/big12-football-players-2025.json'
  );
  result.totalSize += fileSize;

  console.log(
    `  ✅ Saved ${playerProfiles.length} player profiles (${(fileSize / 1024).toFixed(1)} KB)`
  );
  ...
  for (const [school, schoolPlayers] of Object.entries(playersBySchool)) {
    const schoolFile = path.join(
      dataDir,
      `${school.toLowerCase().replace(/\s+/g, '-')}-players-2025.json`
    );
    const schoolData = {
      school,
    ...
    result.filesCreated.push(
      `public/data/media-day/${path.basename(schoolFile)}`
    );
  }

  console.log(
    `  📚 Created ${Object.keys(playersBySchool).length} school-specific files`
  );
}

async function saveCoachProfiles(
  coaches: any[],
  dataDir: string,
  result: ProfileSaveResult
): Promise<void> {
  ...
  const successfulCoaches = coaches.filter(
    (c) => c.success && c.research?.profile
  );
  ...
  const allCoachesData = {
    metadata: {
      ...
      avgConfidence:
        coachProfiles.reduce((sum, c) => sum + c.confidence, 0) /
  ...
  result.filesCreated.push(
    'public/data/media-day/big12-football-coaches-2025.json'
  );
  result.totalSize += fileSize;

  console.log(
    `  ✅ Saved ${coachProfiles.length} coach profiles (${(fileSize / 1024).toFixed(1)} KB)`
  );
}
...
async function saveSummaryData(
  ...
  const summaryFile = path.join(
    dataDir,
    'media-day-research-summary-2025.json'
  );
  const summaryData = {
    generated: new Date().toISOString(),
    researchScope: {
      ...
      successRate:
        (
          (researchResults.summary.successfulProfiles /
            researchResults.summary.totalSubjects) *
          100
        ).toFixed(1) + '%',
    },
    confidence: {
      avgPlayerConfidence:
        (researchResults.summary.avgPlayerConfidence * 100).toFixed(1) + '%',
      avgCoachConfidence:
        (researchResults.summary.avgCoachConfidence * 100).toFixed(1) + '%',
      overallConfidence:
        (
          ((researchResults.summary.avgPlayerConfidence +
            researchResults.summary.avgCoachConfidence) /
            2) *
          100
        ).toFixed(1) + '%',
    },
    completeness: {
      avgPlayerCompleteness:
        (researchResults.summary.avgPlayerCompleteness * 100).toFixed(1) + '%',
      avgCoachCompleteness:
        (researchResults.summary.avgCoachCompleteness * 100).toFixed(1) + '%',
    },
    apiPerformance: {
      geminiSuccessRate:
        (researchResults.summary.geminiSuccessRate * 100).toFixed(1) + '%',
      perplexitySuccessRate:
        (researchResults.summary.perplexitySuccessRate * 100).toFixed(1) + '%',
    },
    ...
    processingMetrics: {
      ...
      totalTimeSeconds: (
        researchResults.summary.processingTimeMs / 1000
      ).toFixed(1),
      avgProcessingRate:
        (
          researchResults.summary.totalSubjects /
          (researchResults.summary.processingTimeMs / 1000)
        ).toFixed(2) + ' profiles/second',
    },
  };
  ...
  result.filesCreated.push(
    'public/data/media-day/media-day-research-summary-2025.json'
  );
  result.totalSize += fileSize;

  console.log(
    `  ✅ Saved comprehensive summary (${(fileSize / 1024).toFixed(1)} KB)`
  );
}
...
function calculateAverageTenure(coaches: any[]): number {
  ...
  const tenures = coaches
    .filter(
      (c) => c.profile?.hire_date && c.profile.hire_date !== 'Research Required'
    )
    .map((c) => {
      try {
...
if (require.main === module) {
  saveProfilesToFiles()
    .then((result) => {
      ...
      console.log(
        'All Big 12 Media Day profiles are now stored in permanent files.'
      );
