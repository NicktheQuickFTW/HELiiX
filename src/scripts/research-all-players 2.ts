...
async function researchAllPlayers(): Promise<void> {
  ...
  console.log(`🏫 Schools represented: ${new Set(confirmedPlayers.map(p => p.school)).size}`);
  console.log('');
  ...
  for (const [school, schoolPlayers] of Object.entries(playersBySchool)) {
    ...
    for (let i = 0; i < schoolPlayers.length; i += batchSize) {
      ...
      const batchPromises = batch.map(async (player) => {
        ...
        try {
          ...
          const profile = await playerAgent.researchPlayer(player.name, player.school, player.position);
          ...
          console.log(`   ${status} ${player.name.padEnd(20)} | ${player.position.padEnd(3)} | ${(completeness * 100).toFixed(1)}% | ${(processingTime / 1000).toFixed(1)}s`);
          
          // Show key stats if available
          if (profile.statistics?.current_season) {
            const stats = profile.statistics.current_season;
            if (stats.passing_yards) console.log(`      📊 Passing: ${stats.passing_yards} yards, ${stats.passing_touchdowns} TDs`);
            if (stats.rushing_yards) console.log(`      📊 Rushing: ${stats.rushing_yards} yards, ${stats.rushing_touchdowns} TDs`);
            if (stats.receiving_yards) console.log(`      📊 Receiving: ${stats.receiving_yards} yards, ${stats.receiving_touchdowns} TDs`);
            if (stats.tackles) console.log(`      📊 Defense: ${stats.tackles} tackles, ${stats.sacks || 0} sacks`);
          }
          ...
          const processingTime = Date.now() - playerStartTime;
          console.log(`   ❌ ${player.name.padEnd(20)} | ${player.position.padEnd(3)} | ERROR | ${(processingTime / 1000).toFixed(1)}s`);
      ...
      const resolvedResults = batchResults
        .filter((result): result is PromiseFulfilledResult<PlayerResearchResult> => result.status === 'fulfilled')
    ...
    const avgCompleteness = successCount > 0 
      ? schoolPlayerResults.filter(r => r.profile).reduce((sum, r) => sum + r.completeness, 0) / successCount 
      : 0;
    ...
    console.log(`   📈 ${school} Summary: ${successCount}/${schoolPlayers.length} successful, ${(avgCompleteness * 100).toFixed(1)}% avg completeness, ${(schoolTime / 1000).toFixed(1)}s`);
    console.log('');
  }
  ...
  const overallAvgCompleteness = totalSuccessful > 0
    ? allResults.filter(r => r.profile).reduce((sum, r) => sum + r.completeness, 0) / totalSuccessful
    : 0;
  const avgProcessingTime = allResults.reduce((sum, r) => sum + r.processingTime, 0) / allResults.length;

  console.log(`✅ Successful Profiles: ${totalSuccessful}/${confirmedPlayers.length} (${((totalSuccessful / confirmedPlayers.length) * 100).toFixed(1)}%)`);
  console.log(`❌ Failed Profiles: ${totalFailed}/${confirmedPlayers.length}`);
  console.log(`📈 Average Completeness: ${(overallAvgCompleteness * 100).toFixed(1)}%`);
  console.log(`⏱️  Total Processing Time: ${(totalTime / 1000).toFixed(1)} seconds`);
  console.log(`⚡ Average Processing Time: ${(avgProcessingTime / 1000).toFixed(1)}s per player`);
  console.log(`🚀 Processing Rate: ${(confirmedPlayers.length / (totalTime / 1000)).toFixed(2)} players/second`);
  ...
  schoolResults
    ...
    .forEach((school, index) => {
      const successRate = (school.successCount / school.players.length * 100).toFixed(1);
      const completenessScore = (school.avgCompleteness * 100).toFixed(1);
      
      console.log(`${index + 1}. ${school.school.padEnd(15)} | ${school.successCount}/${school.players.length} success | ${completenessScore}% completeness | ${(school.batchTime / 1000).toFixed(1)}s`);
    });
  ...
  topPerformers.forEach((result, index) => {
    console.log(`${index + 1}. ${result.player.name.padEnd(20)} | ${result.player.school.padEnd(15)} | ${result.player.position.padEnd(3)} | ${(result.completeness * 100).toFixed(1)}%`);
  });
  ...
  if (totalFailed > 0) {
    ...
    failedResults.forEach((result, index) => {
      console.log(`${index + 1}. ${result.player.name} (${result.player.school}, ${result.player.position}): ${result.error}`);
    });
  }
...
function calculatePlayerCompleteness(player: any): number {
  ...
  const requiredFields = ['name', 'school', 'position', 'year', 'physical_attributes'];
  const importantFields = ['statistics', 'recruiting_info', 'media_day_talking_points'];
  const optionalFields = ['background', 'social_media', 'achievements', 'injury_status'];
...
async function saveResearchResults(allResults: PlayerResearchResult[], schoolResults: SchoolBatchResult[], summary: any): Promise<void> {
  ...
  console.log(`📁 Research dataset: ${JSON.stringify(fullDataset, null, 2).length} characters`);
  console.log(`📊 Generated breakdown for ${Object.keys(generatePositionBreakdown(allResults)).length} positions`);
  console.log(`🏫 School rankings generated for ${schoolResults.length} schools`);
  
  // Also generate a CSV summary for quick analysis
  const csvHeader = 'School,Player,Position,Year,Completeness,ProcessingTime,Success,Error';
...
function generatePositionBreakdown(results: PlayerResearchResult[]): Record<string, any> {
  ...
  for (const position in breakdown) {
    ...
    const successfulPlayers = positionData.players.filter((p: any) => p.success);
    positionData.avg_completeness = successfulPlayers.length > 0
      ? successfulPlayers.reduce((sum: number, p: any) => sum + p.completeness, 0) / successfulPlayers.length
      : 0;
    positionData.success_rate = positionData.successful_profiles / positionData.total_players;
  }
