...
const mockPlayerProfiles = [
  // Arizona players
  {
    ...
    background:
      'Transfer from Fresno State, known for mobility and arm strength',
    social_media: {
      twitter: '@NoahFifita',
...

  {
    ...
    media_day_talking_points: {
      season_goals:
        'Setting Arizona receiving records and helping team win conference',
      team_dynamics:
        'Building rapport with quarterback and being a reliable target',
    ...
    achievements: [
      ...
      'Arizona single-season receptions leader',
    ],
  ...
  // Colorado players (high-profile)
  {
    ...
    media_day_talking_points: {
      season_goals:
        'Leading Colorado to College Football Playoff consideration',
      team_dynamics:
        'Continuing to build with Coach Prime and the culture change',
      personal_development:
        'Becoming a more vocal leader and improving pocket presence',
      conference_competition:
        'Proving Colorado belongs in Big 12 championship conversation',
    },
    achievements: [
      '2023 All-Pac-12 First Team',
...

  {
    ...
    media_day_talking_points: {
      ...
      team_dynamics:
        'Being the leader Coach Prime expects on both sides of the ball',
      personal_development:
        'Perfecting technique as both receiver and cornerback',
      conference_competition:
        'Excited to showcase two-way ability against Big 12 competition',
    },
    achievements: [
      '2023 AP All-American',
...
async function validateAllPlayerProfiles(): Promise<void> {
  ...
  for (let i = 0; i < Math.min(3, mockPlayerProfiles.length); i++) {
    ...
    console.log(
      `   Required Fields: ${(report.completenessMetrics.requiredFields * 100).toFixed(1)}%`
    );
    console.log(
      `   Optional Fields: ${(report.completenessMetrics.optionalFields * 100).toFixed(1)}%`
    );
    console.log(
      `   Data Quality: ${(report.completenessMetrics.dataQuality * 100).toFixed(1)}%`
    );
    console.log(
      `   Consistency: ${(report.completenessMetrics.consistency * 100).toFixed(1)}%`
    );
  }
