...
function displayCoachResults(results: any) {
  ...
  console.log(`⏱️  Processing Time: ${(results.summary.processingTime / 1000).toFixed(2)} seconds`);
  console.log(`📈 Average Completeness: ${(results.summary.avgCompleteness * 100).toFixed(1)}%`);
  ...
  if (results.coaches && results.coaches.length > 0) {
    results.coaches.forEach((coach: any, index: number) => {
      ...
      if (coach.career_record?.overall_wins !== undefined) {
        console.log(`    📊 Record: ${coach.career_record.overall_wins}-${coach.career_record.overall_losses}`);
      }
      
      if (coach.hire_date) {
        const tenure = new Date().getFullYear() - new Date(coach.hire_date).getFullYear();
        console.log(`    📅 Tenure: ${tenure} years (hired ${coach.hire_date})`);
      }
...
function displaySystemPerformance(report: any) {
  ...
  console.log(`⚡ Average Processing: ${report.summary?.avgProcessingTime?.toFixed(2) || 'N/A'}ms per profile`);
  console.log(`🎯 System Availability: ${(report.summary?.availability * 100).toFixed(2)}%`);
  ...
  if (report.scalingEvents && report.scalingEvents.length > 0) {
    ...
    report.scalingEvents.forEach((event: any) => {
      console.log(`   ${event.type.toUpperCase()}: ${event.previousInstances} → ${event.newInstances} instances (${event.reason})`);
    });
  }
}
...
async function saveCoachProfiles(results: any) {
  ...
  console.log(`📁 Profile data saved: ${JSON.stringify(results, null, 2).length} characters`);
  ...
  if (results.coaches && results.coaches.length > 0) {
    ...
    const csvRows = results.coaches.map((coach: any) => {
      ...
      const record = coach.career_record ? 
        `${coach.career_record.overall_wins}-${coach.career_record.overall_losses}` : 'N/A';
