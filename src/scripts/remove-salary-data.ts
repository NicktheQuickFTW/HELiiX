...
async function cleanupCoachResearchAgent(
  result: ProfileCleanupResult
): Promise<void> {
  console.log('🔧 Cleaning up Coach Research Agent...');

  const filePath =
    '/Users/nickw/Documents/XII-Ops/HELiiX/src/lib/agents/research/coach-research-agent.ts';
  ...
  try {
    ...
    if (content.includes('contract_details')) {
      ...
      content = content.replace(
        /total_compensation.*,/g,
        '// total_compensation removed,'
      );
      modified = true;
...
async function cleanupDeepResearchAgent(
  result: ProfileCleanupResult
): Promise<void> {
  console.log('🔧 Cleaning up Deep Research Agent...');

  const filePath =
    '/Users/nickw/Documents/XII-Ops/HELiiX/src/lib/agents/research/deep-research-agent.ts';
  ...
  try {
    ...
    if (
      content.includes('annual_salary') ||
      content.includes('total_compensation')
    ) {
      content = content.replace(
        /annual_salary: `\$[^`]*`,/g,
      ...
      content = content.replace(
        /salary: `\$[^`]*`,/g,
        '// salary: "Contract details private",'
      );
      modified = true;
      result.contractsModified++;
    }
    ...
    if (content.includes('usa_today_salary_rank')) {
      content = content.replace(
        /usa_today_salary_rank:.*,/g,
        '// usa_today_salary_rank: removed,'
      );
      modified = true;
    }
...
async function cleanupValidationAgent(
  result: ProfileCleanupResult
): Promise<void> {
  console.log('🔧 Cleaning up Validation Agent...');

  const filePath =
    '/Users/nickw/Documents/XII-Ops/HELiiX/src/lib/agents/validation/profile-validation-agent.ts';
  ...
  try {
    ...
    if (content.includes('salary')) {
      content = content.replace(
        /field: 'contract_details\.salary'[^}]*}/s,
...
async function cleanupGeneratedProfiles(
  result: ProfileCleanupResult
): Promise<void> {
  ...
  for (const filePath of scriptsToClean) {
    try {
      if (fs.existsSync(filePath)) {
        ...
        if (
          content.includes('$5.5M annually') ||
          content.includes('$4.0M annually')
        ) {
          content = content.replace(
            /\$\d+\.\d+M annually/g,
            'Contract details private'
          );
          modified = true;
          result.contractsModified++;
        }
