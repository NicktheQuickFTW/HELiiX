...
async function demonstrateCoachProfileGeneration() {
  ...
  for (let i = 0; i < big12Coaches.length; i += batchSize) {
    ...
    const batchPromises = batch.map(async (coach) => {
      try {
        ...
        const profile = await coachAgent.researchCoach(
          coach.name,
          coach.school
        );
        ...
        if (profile.career_record?.overall_wins !== undefined) {
          console.log(
            `      📊 Record: ${profile.career_record.overall_wins}-${profile.career_record.overall_losses}`
          );
        }
        ...
        if (profile.hire_date) {
          ...
          console.log(
            `      📅 Tenure: ${tenure} years (hired ${profile.hire_date})`
          );
        }
        ...
        console.error(
          `  ❌ Failed to generate profile for ${coach.name}:`,
          error.message
        );
  ...
  const avgCompleteness =
    successful.length > 0
      ? successful.reduce((sum, r) => sum + r.completeness, 0) /
        successful.length
      : 0;

  console.log(
    `✅ Successful: ${successful.length}/${big12Coaches.length} coaches`
  );
  ...
  console.log(
    `⏱️  Total Processing Time: ${(processingTime / 1000).toFixed(2)} seconds`
  );
  console.log(
    `⚡ Processing Rate: ${(big12Coaches.length / (processingTime / 1000)).toFixed(2)} profiles/second`
  );
  ...
  if (failed.length > 0) {
    ...
    failed.forEach((result, index) => {
      console.log(
        `   ${index + 1}. ${result.coach.name} (${result.coach.school}): ${result.error}`
      );
    });
  }
  ...
  topCoaches.forEach((result, index) => {
    console.log(
      `${index + 1}. ${result.coach.name} (${result.coach.school}) - ${(result.completeness * 100).toFixed(1)}%`
    );
  });
...
if (require.main === module) {
  demonstrateCoachProfileGeneration()
    .then(() => {
      console.log('');
      console.log(
        '🎉 Big 12 Coach Profile Generation Demo Completed Successfully!'
      );
      process.exit(0);
    })
    .catch((error) => {
